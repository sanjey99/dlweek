  ...
  for (const part of parts) {
    ...
    if (part?.type === 'text') {
      ...
      if (textVal.includes('/mnt/data/')) {
        // Presence of sandbox path is a strong hint that the assistant created a file;
        // fallback to null if no explicit file_id is discoverable.
        continue;
      }
    }
  }
...
async function fetchAssistantOutputFile(fileId) {
  ...
  try {
    ...
    console.log(`[DEBUG] New File Created by AI: ${fileId}`);
    console.log('[DEBUG] Streaming file to Jerald...');
...
async function pollRunUntilStable(threadId, runId, {
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    ...
    console.log(`[DEBUG] Active Run Status: ${run.status}`);
...
app.post('/api/agent/chat', async (req, res) => {
  try {
    ...
    // Fixed thread mode to preserve long-lived assistant context for Jerald.
    ...
    try {
      ...
      const uploadContextMessage = hasUploadedFileContent
        ? `Jerald has uploaded ${file.name || 'attached-context.txt'}. The file is attached for Code Interpreter access.\n\nJerald request: ${userInput}`
        : `Jerald says: ${userInput}`;
    ...
    const runCreateParams = {
      ...
      ...(isLowRiskDirectExecute
        ...
        : {
            // For MODERATE/HIGH risk, enforce governance tool call path.
            tool_choice: { type: 'function', function: { name: 'propose_file_edit' } },
            additional_instructions: `ML interceptor marked this request MODERATE/HIGH risk. If and only if Jerald explicitly requests a file modification, call propose_file_edit before execution. If Jerald is asking a clarification/general question, respond in text and do not call propose_file_edit. ${EXECUTION_OUTPUT_POLICY}`,
          }),
    };
